...
# How to add Alaska and Hawaii to cartopy map
# From: https://stackoverflow.com/questions/55598249/showing-alaska-and-hawaii-in-cartopy-map
...
def plot_fuel_econ(df, var, axis_label, **kwargs):
    ...
    for astate in shpreader.Reader(states_shp).records():
        ...
        if geo_state in ["Alaska", "Hawaii"]:
            ...
            # add inset maps
            add_insetmap(axes_extent, map_extent, geo_state, \
                         facecolor, \
                         edgecolor, \
                         astate.geometry)
    ...
    cax = fig.add_axes([.45, .12, .4, 0.04]) # Start X, start Y, X width, Y width
    if 'break-even' in kwargs and kwargs['break-even']:
        extend = 'max'
    else:
        extend = 'neither'
